...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Set auth cookie BEFORE constructing the response (Next.js requirement:
    // cookies must be mutated before the response is sent).
    await setAuthCookie({ userId: user.id, username: user.username })

    // Build response with explicit headers to ensure proper serialization.
