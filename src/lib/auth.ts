...
export async function getAuthUser(): Promise<{ id: string; username: string; avatar: string; gamesPlayed: number; gamesWon: number; gamesLost: number; gamesDraw: number } | null> {
  try {
    const cookieStore = await cookies()
